...
from const import BANNED_RAW_FEATURE_KEYS_OUTPUT
import json


if __name__ == '__main__':
    logger.add("feature_extraction_v10_predict_raw_features.log", rotation="10 MB")
    ...
    ckpt_rootname = "highlight_model_{}_extracted_features_all_data_predict_raw_feature_baseline{}{}".format(
        model_name,
    ...
    if not os.path.exists(ckpt_prompt_name):
        ...
        for key in dataset.features:
            if key in BANNED_RAW_FEATURE_KEYS_OUTPUT:
                continue
            all_vals = set([str(x) for x in dataset[key] if x is not None])
            if len(all_vals) == 0:
                continue
            if len(all_vals) == 1:
                # remove those ever non-existing features
                if list(all_vals)[0] in ['False', "None", "[]"]:
                    continue
            # schema_dict[key] = dataset.features[key]
            # # randomly sample some values from dataset[key]
            # schema_dict[key].example = random.sample(list(all_vals), min(10, len(all_vals)))
            schema_dict[key] = {
                "type": dataset.features[key].dtype if not isinstance(dataset.features[key], list) else "list",
                "example": random.sample(list(all_vals), min(10, len(all_vals)))
            }
        ...
        all_output_features = list(schema_dict.keys())
        ...
        for desc_i, desc in enumerate(descriptions):
            ...
            for category in schema_dict:
                ...
                if dataset[desc_i][category] is None:
                    continue
                if isinstance(dataset[desc_i][category], list):
                    if len(dataset[desc_i][category]) == 0:
                        continue
                if schema_dict[category]['type'] == "bool":
                    if not dataset[desc_i][category]:
                        continue
                sampled_values = schema_dict[category]['example']
                total_prompts.append(
                    "Your task is to determine whether the given label class category is mentioned in the description. The meaning of the category will be explained by example values of this category. Only respond by 'YES' or 'NO'. "
                    "Label Class Category: {}. \n\nExample Values in this Category: {}\n\n"
                    "\n\nDescription: {}\n\nResponse (Yes/No): ".format(category, sampled_values, desc))
            id2range[ids[desc_i]] = (start_id, len(total_prompts))
        torch.save([total_prompts, scores, schema_dict, id2range, all_output_features], ckpt_prompt_name)
    ...
    for i in range(len(outputs), len(prompts), args.checkpoint_freq):
        ...
        #     batch_scores = scores[i: i + args.checkpoint_freq]
        # else:
        #     batch_scores = None
        ...
        for output_inst_i, output_inst in enumerate(batch_outputs):
            new_output_item = {
                ...
                # "score": batch_scores[output_inst_i] if batch_scores is not None else None
            }
